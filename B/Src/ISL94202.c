...
static uint8_t CurrentVoltageBuffer[22];
//static uint8_t EEPROMScratch[4]; //Used to buffer the user EEPOM [used for cal data]
                                     ...
                                     uint8_t sleepModeTimeout)
{
    ...
    uint8_t cellMask = 0x83; //0b10000011;
    if (cellCount == 4)
        cellMask =     0xC3; //0b11000011;
    if (cellCount == 5)
        cellMask =     0xC7; //0b11000111;
    if (cellCount == 6)
        cellMask =     0xE7; //0b11100111;
    if (cellCount == 7)
        cellMask =     0xEF; //0b11101111;
    if (cellCount == 8)
        cellMask =     0xFF; //0b01111111;
...
uint16_t ISL94202_getPackCurrentMA(uint32_t divisor)
{
...
//Read from 0x80 5 bytes
    I2C_readMany(slaveAdd, statusAdd, 5, CurrentStatusBuffer);
}

void ISL94202_updateReadings()
{
//read from 0x8A 22 bytes
    I2C_readMany(slaveAdd, voltageAdd, 22, CurrentVoltageBuffer);
}
...
void enableEEPROMAccess()
{
    ...
    I2C_writeReg8(slaveAdd, 0x89, 0x01);
}
void disableEEPROMAccess()
{
	
    //Set the bit to change to read/write EEPROM
    I2C_writeReg8(slaveAdd, 0x89, 0x00);
}
...
void ISL94202_writeEEPROMWord(uint8_t reg, uint16_t value)
{
    ...
    I2C_writeReg8(slaveAdd, base, buffer[0]);
    HAL_Delay(3500);    //pause for EEPROM write
...
uint8_t ISL94202_readUserEEPROM(uint8_t address)
{
    ...
    do
    {
        r = I2C_readReg8(slaveAdd, 0x50 + (address & 0x0F));
        HAL_Delay(10);    //1ms
        r2 = I2C_readReg8(slaveAdd, 0x50 + (address & 0x0F));
